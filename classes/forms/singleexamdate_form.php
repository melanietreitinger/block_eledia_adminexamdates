...
class singleexamdate_form extends \moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('date_time_selector', "blocktimestart",
                get_string('block_timestart', 'block_eledia_adminexamdates'));
        $mform->addRule('blocktimestart', null, 'required', null, 'client');

        $mform->addElement('text',
                "blockduration",
                get_string('block_duration', 'block_eledia_adminexamdates'),
                ['size' => 4]);
        ...
        /* $options=[];
         ...
         $mform->addRule('examname', null, 'required');*/ // $blocks =& $this->_customdata['blocks'];
        ...
        // for($i = 0; $i < $blocksnumber; $i++) {
        foreach ($options as $key => $val) {
            $checkboxes[] = $mform->createElement('advcheckbox', "blockexamroomscheck[{$key}]", '', $val);
        }
        // }
        $mform->addGroup($checkboxes, '',
                get_string('examdaterooms', 'block_eledia_adminexamdates'), ['<br>'], false);
        ...
        $autocompleteoptions = ['multiple' => true, 'tags' => true,
        ...
        foreach ($options as $key => $val) {
            ...
            if (in_array($key, $roomcapacity)) {
                ...
                $mform->addElement('text',
                        "roomnumberstudents[{$key}]",
                        get_string('room_number_students', 'block_eledia_adminexamdates'),
                        ['size' => 4]);
